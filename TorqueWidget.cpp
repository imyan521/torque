...
float TorqueWidget::TORQUE_ZERO = 0.019f;
float TorqueWidget::ANGLE_ZERO = 205.291f;

TorqueWidget::TorqueWidget(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::TorqueWidget),
    ...
    minAngle(0.0)
{
    ...
    // connect(timer, &QTimer::timeout, this, &TorqueWidget::updateSampleData);
    connect(timer, &QTimer::timeout, this, &TorqueWidget::updateTime);
    ...
    // 连接按钮点击信号到槽函数
    connect(ui->startButton, &QPushButton::clicked, this, &TorqueWidget::onStartButtonClicked);
    connect(ui->pauseButton, &QPushButton::clicked, this, &TorqueWidget::onPauseButtonClicked);
    connect(ui->resetButton, &QPushButton::clicked, this, &TorqueWidget::onResetButtonClicked);

    //连接选择设备的点击信号到 Connect_device槽函数
    connect(ui->connect_device,&QPushButton::clicked,this, &TorqueWidget::Connect_device);
    ...
    // 连接信号，当接收到数据时处理UI更新
    //connect(server, &MyServer::bool_data_reciv, this, &TorqueWidget::DataReceived);
    ...
    connect(ui->saveTorqueZeroButton, &QPushButton::clicked, this, &TorqueWidget::onSaveTorqueZeroButtonClicked);
    connect(ui->saveAngleZeroButton, &QPushButton::clicked, this, &TorqueWidget::onSaveAngleZeroButtonClicked);
}

TorqueWidget::~TorqueWidget()
{
    delete ui;
}

void TorqueWidget::onSaveTorqueZeroButtonClicked() {
...
void TorqueWidget::onSaveAngleZeroButtonClicked() {
...
void TorqueWidget::DataReceived(QString str) {
...
void TorqueWidget::updatePlots(bool newDataReceived) {
...
void TorqueWidget::updateSampleData() {
...
// 保存数据的函数
void TorqueWidget::saveData(const QString &fileName, const QString &format) {
    QFile file(fileName);
...
// 点击开始按钮后
void TorqueWidget::onStartButtonClicked() {
...
// 点击暂停按钮后
void TorqueWidget::onPauseButtonClicked() {
...
// 归零按钮的槽函数
void TorqueWidget::onResetButtonClicked() {
    // 停止并重置计时器
    timer.stop();
    timeElapsed = 0.0;
...
//显示设备连接界面
void TorqueWidget::Connect_device(){
